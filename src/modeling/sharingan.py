...
class SharinganModule(pl.LightningModule):
    def __init__(self, cfg):
        ...
        elif self.dataset == "goo":
            self.num_train_samples = cfg.data.goo.num_train_samples
        elif self.dataset == "goo_synth":
            self.num_train_samples = cfg.data.goo_synth.num_train_samples
        elif self.dataset == "experiment":
            self.num_train_samples = cfg.data.experiment.num_train_samples
...
class ConditionalDPTDecoder(nn.Module):
    def __init__(
        ...
        self.use_bn = use_bn
        self.heatmap_size = heatmap_size  # << 추가
    ...
    def forward(self, x):
        ...
        z = F.interpolate(z, size=(self.heatmap_size, self.heatmap_size), mode="bilinear", align_corners=False)  # (b*n, 1, H, W) > (b*n, 1, 64, 64)
        z = z.view(b, n, self.heatmap_size, self.heatmap_size)  # (b*n, 1, 64, 64) > (b, n, 64, 64)
        return z
